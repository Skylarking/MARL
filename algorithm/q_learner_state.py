...
class QLearnerWithState():
    ...
    def train(self, batch, train_step):
        ...
        q_evals, returns = self.eval_net.get_current_q_values(batch, self.max_episode_len)    # q_eval(n_episodes, max_episode_len, n_agents, n_actions)
        ...
        q_targets, returns_target = self.target_net.get_next_q_values(batch, self.max_episode_len)     # q_targets(n_episodes, n_agents, n_actions)
        ...
        r_pred = returns["r"]
        o_next_pred = returns["o_next"]
        t_pred = returns["terminated"]


        # s_label = th.repeat_interleave(s_next, self.args.n_agents, dim=1).reshape(-1, self.max_episode_len, self.args.n_agents, self.args.state_shape)
        loss_pred = ((o_next.detach() - o_next_pred) ** 2).mean()
            ...
            for i in range(3):  # agent 0 action
                for j in range(3):  # agent 1 action
                    ...
                    if self.args.alg == 'qplex':
                        # calculate v_tot
                        v_tot = self.mixer(q_values_chosen, one_transition['s'].cuda(), is_v=True)
                        ...
                        a_tot = self.mixer(q_values_chosen, one_transition['s'].cuda(), actions=chosen_actions_one_hot, max_q_i=max_action_qvals, is_v=False)  # 计算优势值A
                        # calculate q_tot
                        q_tot_table[i, j] = v_tot + a_tot
                    else: # VDN & QMIX
                        q_tot_table[i, j] = self.mixer(q_values_chosen, one_transition['s'].cuda()).item()
