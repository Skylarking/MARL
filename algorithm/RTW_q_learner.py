...
class RTWQLearner():
    ...
    def train(self, batch, train_step):
        ...
        q_evals, _, loss1_mean, loss2_mean = self.eval_net.get_current_q_values(batch, self.max_episode_len)    # q_eval(n_episodes, max_episode_len, n_agents, n_actions)
        ...
        self.logger.log_stat("td_loss", loss.cpu().item(), train_step)
        self.logger.log_stat("teammate_loss", loss1_mean.cpu().item(), train_step)
        self.logger.log_stat("world_loss", loss2_mean.cpu().item(), train_step)
        loss = loss + loss1_mean + loss2_mean
        # print('Loss is ', loss)
